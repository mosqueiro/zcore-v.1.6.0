...
- [Building ZCore](#building-zcore-core)
...
Clone the git repositories for ZCore and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zcorecoin/zcore
```
...
in the ZCore repository under 'Fetch and build inputs' to install sources which require
...
To build ZCore (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ZCore repository.
...
```bash
    ...
    From https://github.com/zcorecoin/zcore
...
and the zcore git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zcore/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zcore.git
SIGPATH=/some/root/path/zcore-detached-sigs.git

./bin/gbuild --url zcore=${BTCPATH},signature=${SIGPATH} ../zcore/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zcore-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zcore-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zcore-osx-build.assert
```
...
[zcore/gitian.sigs](https://github.com/zcorecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
